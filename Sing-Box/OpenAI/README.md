# OpenAI

由 SubPilot 自动维护；只生成本规则集需要的文件。

Automatically maintained by SubPilot; only files needed by this rule set are generated.

| 文件 / File | 用途 / Purpose |
| --- | --- |
| [ip-ranges.srs](./ip-ranges.srs) | IP 网段路由规则 / IP range routing rules |
| [routing.srs](./routing.srs) | 路由匹配规则 / Routing rules |
| [dns-domains.srs](./dns-domains.srs) | DNS 匹配域名 / DNS matching domains |
| [manifest.json](./manifest.json) | 发布校验信息 / Publication receipt |

[全部规则集 / All rule sets](../)
